...
router.post("/register/candidate", async (req, res) => {
  try {
    ...
    // duplicate check (use normalized email)
    const existingCandidate = await Candidate.findOne({ email: normEmail }).lean();
    const existingEmployer = await Employer.findOne({ email: normEmail }).lean();
    if (existingCandidate || existingEmployer) {
      return res.status(400).json({ message: "Email already registered" });
    }
    ...
    // Create verification record and send OTP to email
    try {
      const verificationId = await createVerificationRecord({ value: normEmail });
      return res.status(201).json({ message: "Candidate registered. OTP sent to email.", verificationId });
    } catch (sendErr) {
      // rollback created candidate to avoid orphan unverified accounts
      try {
...
router.post("/send-otp", async (req, res) => {
  try {
    ...
    const normValue = normalizeEmail(value);
    const verificationId = await createVerificationRecord({ value: normValue });
    return res.json({ message: "OTP sent to email", verificationId });
  } catch (err) {
    console.error("send-otp error:", err);
    const errMsg = err?.response?.body ? JSON.stringify(err.response.body) : err.message || String(err);
...
router.post("/verify-otp", async (req, res) => {
  try {
    ...
    if (verificationId) {
      ...
      query = { type: "email", value: normalizeEmail(value) };
    }
    ...
    // Mark the corresponding user (Candidate or Employer) as verified if present
    const emailValue = ver.value; // already normalized
    let user = await Candidate.findOne({ email: emailValue });
    if (user) {
      user.isVerified = true;
      await user.save();
...
router.post("/login", async (req, res) => {
  try {
    ...
    let user = await Candidate.findOne({ email: emailNorm });
    let role = "candidate";

    if (!user) {
      user = await Employer.findOne({ email: emailNorm });
      role = "employer";
    }

    if (!user) return res.status(400).json({ message: "User not found" });

    // IMPORTANT: do NOT auto-send OTP here. Just block login until verified.
    if (!user.isVerified) return res.status(403).json({ message: "Verify account first (check your email for OTP)" });
