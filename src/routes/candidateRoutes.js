...
// resume removed – only valid types below
const validFileTypes = ["photo", "video", "audio", "documents", "photos"];
...
router.post(
  ...
  (req, res, next) => {
    ...
    req.params.userType = "Candidate"; // for FileUpload controller
    next();
  },
  uploadFile
);
...
router.get("/uploads", authMiddleware, async (req, res) => {
  try {
    ...
    res.json({
      ...
      uploads: candidate.uploads || [], // multiple images + pdfs
    });
  } catch (err) {
    res.status(500).json({ message: "Server error", error: err.message });
  }
...
router.get("/admin/uploads", authMiddleware, async (req, res) => {
  try {
    ...
    const candidates = await Candidate.find().select(
      "firstName lastName email mobile photoUrl videoUrl audioUrl uploads isPaid createdAt updatedAt"
    );
